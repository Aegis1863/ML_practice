...
def picture_return(
    return_list: list, policy_name: str, env_name: str, move_avg: int = 10
):
    """生成图像
    ...
    if move_avg:
        return_list.rolling(window=move_avg).mean().plot(
            label="%d次移动平均" % move_avg, linewidth=1.5
        )
...
def train_on_policy_agent(
    ...
    for epoch in range(s_epoch, total_epochs):
        with tqdm(
            total=(total_episodes - s_episode),
            desc="<%d/%d>" % (epoch + 1, total_epochs),
            leave=False,
        ) as pbar:
            for episode in range(s_episode, total_episodes):
                ...
                if (episode + 1) % 10 == 0:
                    pbar.set_postfix(
                        {
                            "episode": "%d" % (total_episodes * epoch + episode + 1),
...
def show_gym_policy(
    ...
    """
    `env_name`: 环境名称;\\
    `model`: 演员评论员传入带有take_action的类即可;\\
    `render_mode`: 渲染模式;\\
    `epochs`: 展示轮数\\
    `steps`: 每轮多少步\\
        ...
        for i in range(epochs):  # 测试轮数
            ...
            if (i + 1) % 5 == 0:
                pbar.set_postfix(
                    {
                        "epoch": "%d" % (i),
                        "recent_return": "%.3f" % np.mean(test_list[-5:]),
                    }
                )
            pbar.update(1)
    env.close()
